...
public interface HistoryBalanceRepository extends JpaRepository<HistoryBalanceEntity, Integer> {
	...
	@Query(value="SELECT history_balance.* ,user.email FROM `history_balance` INNER JOIN user_balance ON history_balance.user_balance_id=user_balance.user_balance_id INNER JOIN user ON user_balance.user_id=user.user_id ORDER BY history_balance_id",nativeQuery = true)
	List<HistoryBalanceEntity> findAll2();
	
	@Query(value="SELECT history_balance.* ,user.email FROM `history_balance` INNER JOIN user_balance ON history_balance.user_balance_id=user_balance.user_balance_id INNER JOIN user ON user_balance.user_id=user.user_id WHERE user.user_id=?",nativeQuery = true)
